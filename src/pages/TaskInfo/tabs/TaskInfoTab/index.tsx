...
const TaskinfoTab = () => {
  ...
  const [field, setField] = useState(
    JSON.parse(localStorage.getItem("timelines") || "[]").find((d: any) => d.TimelineName === localStorage.getItem("timelineNow"))
  );
        ...
        JSON.parse(localStorage.getItem("timelines") || "[]").map((d: any) => {
          if (d.TimelineName === localStorage.getItem("timelineNow")) {
            d.MainField = field.MainField.map((d: any) => {
              if (d.slug === dataobj.slug) return dataobj;
              if (d.subTask.length === 0) return d;
              if (d.subTask.length !== 0) {
                d.subTask = d.subTask.map((d1: any) => mapArry(d1));
                return d;
        ...
        <div className="row">
            ...
            <table>
              ...
              <tr>
                ...
                <td className="d-flex ">
                  {!isDescChange ? (
                    <>
                      START: {JSON.stringify(dataobj?.startTime || "")}
                      <br />
                      END &nbsp;&nbsp;&nbsp;:
              ...
              <tr>
                <td width={"30%"}>
                  DESC{" "}
                  {!isDescChange ? <AiOutlineEdit onClick={() => setIsDescChange(true)} /> : <AiFillSave onClick={() => setIsDescChange(false)} />}{" "}
                </td>
                <td>
                  {isDescChange ? (
                    <input
          ...
          <div className="col-md-4 d-flex ">
            ...
            <div className="justify-content-center">
              <img
                src={dataobj.image ? `${baseURLProj}${dataobj.image}` : `${baseURLProj}/image/TaskImage/task1683175039380.png`}
                width={"300px"}
                height={"300px"}
          ...
          {dataobj.Comment.map((d: any, i: any) => (
            <div className="row mt-1">
              <div className="col-1 mt-1">
                <img src={`${baseURLProj}${d.image}`} width={80} height={50} />
              </div>
